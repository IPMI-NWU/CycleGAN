...
def weights_init_normal(m):
    ...
    if classname.find("Conv") != -1:
        torch.nn.init.normal_(m.weight.data, 0.0, 0.02)  # 初始化正态分布 torch.nn.init.normal_(tensor, mean=0, std=1)
        if hasattr(m, "bias") and m.bias is not None:
            torch.nn.init.constant_(m.bias.data, 0.0)  # 固定值 torch.nn.init.constant_(tensor, val)
